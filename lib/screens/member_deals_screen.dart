...
class MemberDealsScreen extends StatefulWidget {
  const MemberDealsScreen({super.key});

  @override
  State<MemberDealsScreen> createState() => _MemberDealsScreenState();
}

class _MemberDealsScreenState extends State<MemberDealsScreen> {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      bool? confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(_editingDealId != null 
            ? 'Are you sure you want to update this Member Deal?' 
            : 'Are you sure you want to add this new Member Deal to the website?'),
          actions: [
            TextButton(
      ...
      try {
        ...
        if (_selectedImageBytes != null) {
          final storageRef = FirebaseStorage.instance.ref().child('member_deals/${DateTime.now().millisecondsSinceEpoch}_${_selectedImageName}');
        ...
        if (_editingDealId != null) {
          print("DEBUG: Updating Member Deal...");
            await FirebaseFirestore.instance.collection('member_deals').doc(_editingDealId).update(dealData).timeout(const Duration(seconds: 10), onTimeout: () { throw Exception("Firestore connection timed out! Make sure rules are correct and you are online."); });
            print("DEBUG: Updated successfully.");
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Member Deal Updated Successfully!')));
        } else {
          dealData['createdAt'] = FieldValue.serverTimestamp();
          print("DEBUG: Adding Member Deal...");
          await FirebaseFirestore.instance.collection('member_deals').add(dealData).timeout(const Duration(seconds: 10), onTimeout: () { throw Exception("Firestore connection timed out! Make sure rules are correct and you are online."); });
          print("DEBUG: Added successfully.");
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Member Deal Scheduled Successfully!')));
        }
  ...
  Future<void> _confirmDelete(String id, String title) async {
    ...
    if (confirm == true) {
      await FirebaseFirestore.instance.collection('member_deals').doc(id).delete();
              ...
              child: Column(
                ...
                children: [
                  ...
                  Expanded(
                    ...
                    if (docs.isEmpty) return const Center(child: Text('No Member Deals found.'));
