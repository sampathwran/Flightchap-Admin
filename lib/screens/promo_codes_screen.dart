    ...
    if (_formKey.currentState!.validate()) {
      ...
      bool? confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(_editingDealId != null 
            ? 'Are you sure you want to update this Promo Code?' 
            : 'Are you sure you want to add this new Promo Code to the website?'),
          actions: [
            TextButton(
      ...
      try {
        ...
        if (_editingDealId != null) {
          print("DEBUG: Updating Promo Code...");
            await FirebaseFirestore.instance.collection('promo_codes').doc(_editingDealId).update(dealData).timeout(const Duration(seconds: 10), onTimeout: () { throw Exception("Firestore connection timed out! Make sure rules are correct and you are online."); });
            print("DEBUG: Updated successfully.");
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Promo Code Updated Successfully!')));
        } else {
          dealData['createdAt'] = FieldValue.serverTimestamp();
          print("DEBUG: Adding Promo Code...");
          await FirebaseFirestore.instance.collection('promo_codes').add(dealData).timeout(const Duration(seconds: 10), onTimeout: () { throw Exception("Firestore connection timed out! Make sure rules are correct and you are online."); });
          print("DEBUG: Added successfully.");
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Promo Code Scheduled Successfully!')));
        }
