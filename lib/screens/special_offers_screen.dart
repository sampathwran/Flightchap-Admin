...
class _SpecialOffersScreenState extends State<SpecialOffersScreen> {
    ...
    if (_formKey.currentState!.validate() && _selectedEndTime != null && hasImage) {
      ...
      bool? confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(_editingDealId != null 
            ? 'Are you sure you want to update this Special Offer?' 
            : 'Are you sure you want to add this new Special Offer to the website?'),
          actions: [
            TextButton(
      ...
      try {
        ...
        if (_selectedImageBytes != null) {
          final storageRef = FirebaseStorage.instance.ref().child('special_offers/${DateTime.now().millisecondsSinceEpoch}_${_selectedImageName}');
        ...
        if (_editingDealId != null) {
          await FirebaseFirestore.instance.collection('special_offers').doc(_editingDealId).update(dealData);
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Special Offer Updated Successfully!')));
        } else {
          dealData['createdAt'] = FieldValue.serverTimestamp();
          await FirebaseFirestore.instance.collection('special_offers').add(dealData);
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Special Offer Scheduled Successfully!')));
        }
  ...
  Future<void> _confirmDelete(String id, String title) async {
    ...
    if (confirm == true) {
      await FirebaseFirestore.instance.collection('special_offers').doc(id).delete();
